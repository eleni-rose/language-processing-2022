...
def main():
   ...
   if grade:
      print(time.strftime('%c') + '%34s' % 'Eleni Kokkosis')
...
def program():
   while token.category in [NAME, PRINT]:
    stmt()
   if token.category != EOF:
    raise RuntimeError('Expecting end of file')          # missing code

def stmt():
   simplestmt()
   consume(NEWLINE)          # missing code
...
def simplestmt():
   if token.category == NAME:
     ...
     raise RuntimeError('Expecting stmt')          # missing code
...
def printstmt():
   ...
   consume(LEFTPAREN)
   expr()
   consume(RIGHTPAREN)         # missing code
